...
#' bc_transform(-5:5, 1)
#' 
bc_transform <- function(x, lambda = 0){
  # check if x is numeric
  if(any(!is.numeric(x))){
    stop("Box-Cox transformation requires a vector of numbers as an input")
  }
  
  # if any value in x is negative, add delta = |min| + 1% to assure all values are positive
  if(any(x <= 0)){
    delta = abs(min(x)) + 0.01 * abs(min(x))
    x <- x + delta
  }
